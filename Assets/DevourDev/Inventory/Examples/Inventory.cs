...
{
    public sealed class Inventory : IInventory<ItemSo, ItemData, InventorySlot>
    {
        ...
        public Inventory(int slotsCount)
        {
            ...
            if (_slotsBuffer == null || _slotsBuffer.Length < slotsCount) //toremove: implement dynamic expanding
            {
                ...
                for (int i = 0; i < _precalculatedInstructionsBuffer.Length; i++)
                {
                    _precalculatedInstructionsBuffer[i] = new();
                }
            }
        ...
        public event System.Action<IInventory<ItemSo, ItemData,InventorySlot>,
            IItemAmount<ItemSo, ItemData>> OnItemsAdded;

        public event System.Action<IInventory<ItemSo, ItemData, InventorySlot>,
            IItemAmount<ItemSo, ItemData>> OnItemsRemoved;
        ...
        public int Add(IItemAmount<ItemSo, ItemData> itemAmount) => Add(itemAmount.Item, itemAmount.Amount);
        public int Add(ItemData item, int amount) => AddInternal(item, amount, true);

        public int Remove(IItemAmount<ItemSo, ItemData> itemAmount) => Remove(itemAmount.Item, itemAmount.Amount);
        ...
        private int AddInternal(ItemData itemData, int amount, bool addAllOrNothing)
        {
            ...
            if (itemData.Reference is IStackableItem stackableItem && stackableItem.StackSize > 1)
            {
            ...
            if (canAddAll || !addAllOrNothing)
            {
                ...
                OnItemsAdded?.Invoke(this, new ReadOnlyItemAmount<ItemSo, ItemData>(itemData, canAdd));
            }
            else
            {
                //optionally
        ...
        private int RemoveInternal(ItemData itemData, int amount, bool removeAllOrNothing)
        {
            ...
            {
                ExecuteInstructions(instructionsCount);
                OnItemsRemoved?.Invoke(this, new ReadOnlyItemAmount<ItemSo, ItemData>(itemData, canRemove));
            }
            else
            {
                //optionally
        ...
        private void EmulateAddingStackable(ItemData itemData, ref int amountLeft, ref int instructionsCount)
        {
            ...
            if (itemData.Reference is IStackableItem stackable)
